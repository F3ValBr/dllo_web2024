...
def agregar_pedido():
    if request.method == 'GET':
        ...
        ped_elegido = request.form['choose-product']
        peds_seleccionados = request.form.getlist('pedidos[]')
        descripcion = request.form.get('descripcion', '')
        region_id = request.form['selectorRegion']
        comuna_id = request.form['selectorComuna']
        nombre = request.form['nombre']
        ...
        contacto_ok = utval.validate_contact_data(nombre, correo, telefono)
        lugar_ok = utval.validate_region_comuna(region_id, comuna_id)
        producto_ok = utval.validate_product_type(ped_elegido)
        descripcion_ok = utval.validate_descripcion(descripcion)
        peds_seleccionados_ok = utval.validate_products(peds_seleccionados)

        if contacto_ok and lugar_ok and producto_ok and descripcion_ok and peds_seleccionados_ok:            
            # guardar en la base de datos
            # insertar el producto
            id_pedido = db.insertar_pedido(
                ped_elegido,
                descripcion,
                comuna_id,
                nombre,
            ...
            # insertar los productos seleccionados
            for id_ped in peds_seleccionados:
                db.insertar_pedido_fruta_verdura(id_pedido, id_ped)

            return jsonify({'success': True, 'message': 'Hemos recibido el registro del pedido. Muchas gracias.'}), 200
        else:
            errores = uthelp.errores_handler(contacto_ok, lugar_ok, producto_ok, descripcion_ok, peds_seleccionados_ok, True)
            mensaje_error = " ".join(errores)
            return jsonify({'success': False, 'message': mensaje_error}), 400
...
def ver_pedidos():
    ...
    peds_pagina = db.get_pedidos_de_a_5(offset)

    data = []
    for pedido in peds_pagina:
        id, tipo, _, comuna_id, nombre_comprador, _, _ = pedido
        tipo = tipo.capitalize()
        lugar = db.get_location_by_id_com(comuna_id)
        comuna, region = lugar[0], lugar[1]
        pedidos = []
        for ped in db.get_ped_verdura_fruta_by_pedido_id(id):
            pedidos.append(ped[0])
        data.append({
            'id': id,
            'tipo': tipo,
            'comuna': comuna,
    ...
    total_pedidos = db.contador_pedidos()
    total_pedidos = int(total_pedidos[0])
    total_pags = (total_pedidos + 4) // 5
    return render_template('ver-pedidos.html', data = data, pagina = pagina, total_pags = total_pags)

@app.route('/informacion-pedido/<int:id>', methods=['GET'])
def informacion_pedido(id):
    pedido = db.get_pedido_by_id(id)
    _, tipo, descripcion, comuna_id, nombre_comprador, email_comprador, celular_comprador = pedido[0]
    tipo = tipo.capitalize()
    lugar = db.get_location_by_id_com(comuna_id)
    comuna, region = lugar[0], lugar[1]
    pedidos = []
